...
assert.match(html, /\? "github-light" : root\.dataset\.defaultTheme/);
...
assert.match(notFound, /<html lang="en" data-default-theme="p2p" data-theme-storage="p2p-services-site-theme">/);
...
for (const image of ["panel.png", "details.png", "editor.png", "compact.png", "grid.png", "general.png", "appearance.png", "services.png", "performance.png", "discovery.png", "packages.png", "bar.png"]) {
  const [width, height] = siteDimensions(image);
  assert.ok(width > 0 && height > 0, `${image} must not be empty`);
  assert.match(html, new RegExp(`src="${image.replace(".", "\\.")}"`));
}
assert.deepEqual(siteDimensions("preview.png"), siteDimensions("panel.png"));
assert.deepEqual(siteDimensions("social-card.png"), [1200, 630]);
