...
ShellRoot {
  ...
  P2PSettingsTransferController {
    ...
    onSucceeded: function(mode, payload) {
      ...
      if (transfer.running || !transfer.undoAvailable) throw new Error("transfer availability was lost after the probe")
      root.stage = 2
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (!transfer.undoAvailable || transfer.activeMode !== "check") throw new Error("undo availability probe did not publish its result")
      root.stage = 1
