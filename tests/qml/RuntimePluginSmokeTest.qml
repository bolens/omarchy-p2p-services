...
ShellRoot {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (widget.allServiceGroupsCollapsed || Object.keys(widget.collapsedServiceGroups).length !== 0) throw new Error("expand-all did not restore visible groups")
      widget.setAllServiceGroupsCollapsed(true)
      if (!widget.allServiceGroupsCollapsed || Object.keys(widget.collapsedServiceGroups).length !== 2) throw new Error("collapse-all did not collapse visible groups")
      widget.activateServiceSelection()
