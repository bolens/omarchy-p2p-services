...
ShellRoot {
  ...
  P2PActionRunner {
    id: runner
    onActionFinished: function(entry, action, exitCode, _detail) {
      if (root.stage === 0) {
        ...
        root.stage = 1
        if (!runner.request({id:"tailscale"}, "stop", ["/usr/bin/false"])) throw new Error("failing service action did not start")
      } else {
