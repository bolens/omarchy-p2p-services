...
ShellRoot {
  ...
  Component.onCompleted: Qt.callLater(function() {
    ...
    if (!presentation || !widgetIcon || !rotation || !density || !groupStyle || !runningRole || !loadingStyle) throw new Error("appearance controls missing")
    ...
    Qt.callLater(function() {
      ...
      if (mockController.events.length !== 10) throw new Error("appearance event count mismatch")
      if (mockController.events[0].barPresentation !== "category-active-total") throw new Error("category presentation event failed")
      if (mockController.events[1].category !== "File sync" || mockController.events[1].icon !== "M") throw new Error("category icon save failed")
      if (mockController.events[2].icon !== "") throw new Error("category icon clear failed")
      if (mockController.events[3].widgetIcon !== "󰒍") throw new Error("blank widget icon fallback failed")
      if (mockController.events[4].barTextRotation !== "clockwise") throw new Error("bar rotation event failed")
      if (mockController.events[5].cardDensity !== "minimal") throw new Error("card density event failed")
      if (mockController.events[6].groupHeaderStyle !== "dense") throw new Error("group style event failed")
      if (mockController.events[7].runningColorRole !== "urgent") throw new Error("theme role event failed")
      if (mockController.events[8].loadingIndicatorStyle !== "glyph" || mockController.events[9].loadingIndicatorGlyph !== "#") throw new Error("loading appearance events failed")
