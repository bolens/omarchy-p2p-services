...
const qmlModule = fs.readFileSync("qmldir", "utf8");
const qmlLintPolicy = fs.readFileSync(".qmllint.ini", "utf8");
const qmlLint = fs.readFileSync("scripts/lint-qml", "utf8");
...
assert.match(runner, /scripts\/lint-qml/);
...
assert.match(qmlLintPolicy, /^InvalidLintDirective=error$/m);
assert.match(workflow, /tests\/run_all\.sh --portable/);
assert.match(workflow, /OMARCHY_SHELL_ROOT=\.omarchy\/shell scripts\/lint-qml/);
